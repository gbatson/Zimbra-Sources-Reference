    ...
    LPCWSTR lpwstrlogonuser, LPCWSTR lpwstrLogonUsrPwd)
{
	...
	{
		//try in builin container
		wstrGroup = _T("LDAP://CN=Domain Admins,CN=Builtin,") + wstrServerDN; 
		dlogi("Failed CN=Users container. Trying with DomainAdmin Group Path: ",wstrGroup);
		if (FAILED(hr = ADsGetObject(wstrGroup.c_str(), IID_IADsGroup, (void **)pGroup.getptr())))
		{
			dlogi("Failed with Builtin container too. Exiting.");
    ...
    lpAdminPassword)
{
	...
	lpwstrStatus = (LPWSTR)MAPIAccessAPI::InitGlobalSessionAndStore(lpMAPITarget, L"EXCH_ADMIN", FL_NONE);
